...
if command -v paramspider >/dev/null 2>&1; then
    ...
    timeout 10m paramspider -d "${TARGET}" -o "${OUTPUT_DIR}/paramspider_results.txt" >> "${LOG_FILE}" 2>&1
fi

# 2. Arjun (Parameter Discovery)
if command -v arjun >/dev/null 2>&1; then
    echo "[+] Running Arjun on main target..." >> "${LOG_FILE}"
    timeout 20m arjun -t 20 -w 2 -u "https://${TARGET}" -oT "${OUTPUT_DIR}/arjun_results.txt" >> "${LOG_FILE}" 2>&1
    
    # Also run on a few interesting URLs from master list
    if [ -f "${URLS_FILE}" ]; then
        grep -v "\*" "${URLS_FILE}" | grep -E "^https?://" | head -n 3 | while read -r url; do
             if [ -n "$url" ]; then
                timeout 5m arjun -t 20 -w 2 -u "$url" -oT "${OUTPUT_DIR}/arjun_$(echo "$url" | md5sum | cut -d' ' -f1).txt" >> "${LOG_FILE}" 2>&1
             fi
        done
    fi
fi
